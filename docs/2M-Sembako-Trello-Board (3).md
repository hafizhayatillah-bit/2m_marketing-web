...
### Card 1: Session-based auth
**Deskripsi:** Implementasikan session-based authentication (Starlette `SessionMiddleware` atau setara). Password admin di-hash pakai `passlib[bcrypt]` — bukan plaintext, bukan hash custom bikin sendiri.
**Definition of Done:** Admin bisa login dengan username+password ter-hash, session tersimpan di signed cookie, password tidak pernah muncul plaintext di log/DB/response manapun.

### Card 2: Halaman login admin + middleware protect route
**Deskripsi:** Buat halaman `/admin/login` (Jinja2 form) dan endpoint proses login, plus dependency/middleware FastAPI yang cek session valid sebelum akses semua route `/admin/*`.
**Definition of Done:** Semua route admin ter-protect; akses tanpa login redirect otomatis ke halaman login; login sukses redirect ke dashboard.

### Card 3: Base layout admin
**Deskripsi:** Bangun base layout Jinja2 (sidebar/nav ke tiap modul: Promo, Branch, Product, News, Logout) yang dipakai berulang di semua CRUD modul nanti. Utility-first — pakai komponen Tailwind standar, jangan invest waktu visual di sini (beda prioritas dari public site).
**Definition of Done:** Base layout siap dipakai, navigasi ke semua modul ada (walau isinya baru nyusul di Day 7-8), logout berfungsi.

### Card 4: Bangun reusable CRUD scaffold
**Deskripsi:** Deliverable paling penting hari ini. Bangun 2 komponen generic:
- **Router factory** di FastAPI: fungsi yang generate route list/create/edit/delete/toggle-active untuk 1 model + schema, tinggal dipanggil ulang per entity.
- **Form macro** Jinja2 generic: render field form berdasarkan definisi field (label, type, required) tanpa nulis HTML form dari nol tiap entity.
**Definition of Done:** Pola scaffold terdokumentasi cara pakainya (komentar kode/README singkat), siap dipanggil untuk Promo (card berikut) dan nanti Branch/Product/News di Day 7-8.

### Card 5: Implementasi Promo CRUD (entity pertama pakai scaffold)
**Deskripsi:** Pasang Promo sebagai entity pertama yang pakai scaffold: create/edit/delete/toggle `is_active`, validasi (harga tidak negatif, `end_date` > `start_date`), upload gambar ke storage cloud (R2/Supabase).
**Definition of Done:** Admin bisa CRUD promo penuh dari UI, validasi jalan, gambar ter-upload & ter-link benar ke record. **Catat waktu pengerjaan card ini** — jadi baseline buat ukur efektivitas scaffold di Day 7.
...
### Card 1: Branch CRUD
**Deskripsi:** Pakai scaffold Day 6, implementasikan CRUD branch dengan field tambahan yang gak ada di Promo: `category_tags` (multi-select dari hardcoded list kategori), `latitude`/`longitude` (input angka, kasih hint di label cara ambil koordinat dari Google Maps), `google_drive_url`, `whatsapp_number`, `operating_hours`.
**Definition of Done:** Admin bisa CRUD 11 data cabang penuh; field koordinat divalidasi dalam range wajar (lat -90 s/d 90, long -180 s/d 180) biar gak ada typo yang ngerusak pin map di Day 5.

### Card 2: Product CRUD
**Deskripsi:** Pakai scaffold, implementasikan CRUD product: `name`, `description`, `image_url` (upload), `category` (dropdown dari hardcoded list yang **sama persis** dengan yang dipakai di filter Product halaman publik Day 4), `is_active`. Form **tidak boleh** punya field harga sama sekali.
**Definition of Done:** Admin bisa CRUD produk; dropdown kategori konsisten dengan yang dipakai di public Product page — kalau list-nya beda, filter Day 4 bakal mismatch dengan data Day 7.

### Card 3: Checkpoint — pastikan scaffold beneran mempercepat
**Deskripsi:** Ini bukan coding baru, tapi validasi: bandingkan waktu pengerjaan Branch & Product CRUD hari ini dengan Promo di Day 6. Kalau ternyata sama lambat atau malah lebih lambat, artinya scaffold belum cukup generic — perlu direfactor **sebelum** lanjut ke News di Day 8, supaya masalah yang sama gak kebawa ke entity ke-4.
**Definition of Done:** Branch & Product CRUD tercatat lebih cepat dikerjakan dibanding Promo (indikator scaffold berhasil). Kalau tidak, ada catatan singkat apa yang bikin lambat untuk diperbaiki sebelum Day 8.
...
### Card 1: News CRUD pakai scaffold
**Deskripsi:** Pakai scaffold Day 6, implementasikan CRUD News: `title`, `content`, `image_url` (upload), `event_date`, `is_active`. Ini entity ke-4 yang pakai scaffold — kalau checkpoint Day 7 nunjukkin scaffold efektif, ini seharusnya jadi yang paling cepat dikerjakan dari semua entity.
**Definition of Done:** Admin bisa CRUD News penuh dari UI; `event_date` tervalidasi format tanggalnya, konsisten dengan urutan ascending yang dipakai di public News page (Day 4).

### Card 2: Wiring image upload — Promo, Product, News
**Deskripsi:** Pastikan ketiga entity yang punya field gambar (Promo, Product, News) beneran menyimpan file ke cloud storage (R2/Supabase), bukan local disk ephemeral, dan URL yang tersimpan di DB adalah URL publik yang valid.
**Catatan:** Kalau upload untuk Promo (Day 6) sudah beres, di sini tinggal reuse fungsi upload yang sama untuk Product & News — jangan tulis ulang logic upload 3 kali.
**Definition of Done:** Upload gambar berfungsi di 3 modul, file tersimpan di cloud storage, gambar tampil benar di halaman publik terkait (Home, Product, News).

### Card 3: Test end-to-end — edit di admin → muncul benar di publik
**Deskripsi:** Uji manual menyeluruh: untuk tiap entity (Promo, Branch, Product, News), buat perubahan lewat admin (create/edit/toggle/delete), lalu verifikasi perubahan itu langsung tercermin di halaman publik terkait — tanpa perlu restart server atau clear cache manual.
**Definition of Done:** Keempat alur sinkronisasi admin→publik terverifikasi jalan benar; temuan bug dicatat untuk diperbaiki di Day 9.
...
### Card 1: Error page 404/500 yang rapi
**Deskripsi:** Buat custom error handler FastAPI untuk 404 (halaman tidak ditemukan) dan 500 (server error) — halaman yang user-friendly, **bukan traceback Python mentah** (ini isu keamanan juga, bisa expose detail internal sistem, bukan sekadar kosmetik).
**Definition of Done:** URL tidak valid menampilkan halaman 404 custom; error internal menampilkan halaman 500 custom tanpa expose stack trace ke user.

### Card 2: Logging dasar
**Deskripsi:** Setup logging dasar (`logging` module Python) untuk mencatat event penting: percobaan login, error 500, aksi create/update/delete di admin. Log ini jadi satu-satunya cara debug production kalau ada masalah, karena lu gak selalu bisa akses server langsung.
**Definition of Done:** Log tercatat dengan format jelas (timestamp, level, pesan), bisa diakses lewat dashboard Railway/Render.

### Card 3: Edge case validation di semua entity
**Deskripsi:** Review ulang validasi input di 4 modul CRUD — cek yang biasa kelewat: form disubmit kosong, format tanggal salah, upload file bukan gambar, angka negatif, string kepanjangan.
**Definition of Done:** Semua modul CRUD menolak input tidak valid dengan pesan error jelas, bukan crash atau silent fail.

### Card 4: pytest — login sukses/gagal
**Deskripsi:** Test yang memastikan login dengan kredensial benar berhasil (session ter-create) dan login dengan kredensial salah ditolak.
**Definition of Done:** 2 test case (sukses & gagal) lolos dijalankan via `pytest`.

### Card 5: pytest — create 1 entity via scaffold
**Deskripsi:** Test create untuk 1 entity (misal Promo) lewat endpoint/scaffold, verifikasi data tersimpan benar di DB test. Karena entity lain pakai pola scaffold yang sama, test ini secara tidak langsung juga jadi bukti scaffold bekerja sesuai desain.
**Definition of Done:** Test create lolos, mengonfirmasi scaffold generic berfungsi benar.

### Card 6: pytest — public endpoint filter is_active
**Deskripsi:** Test yang verifikasi endpoint public (misal `/api/v1/public/promos`) cuma return record dengan `is_active=True`/dalam rentang tanggal valid, dan tidak menampilkan yang nonaktif/expired.
**Definition of Done:** Test lolos; data yang belum waktunya tayang beneran tidak ter-expose ke publik.

### Card 7: README
**Deskripsi:** Tulis README: cara setup project lokal (install dependency, env vars yang dibutuhkan), cara jalankan migration, cara tambah data baru per entity (lewat admin panel, bukan lewat DB manual), dan referensi ke Project Brief untuk konteks arsitektur.
**Kenapa:** Ini artefak yang paling nunjukin skill profesional ke atasan — bukti lu mikirin handover & maintainability, bukan cuma "yang penting jalan di laptop gua".
**Definition of Done:** README lengkap — orang lain (atau lu sendiri 6 bulan lagi) bisa setup project dari nol cuma baca README ini.
...
### Card 1: Connect custom domain company
**Deskripsi:** Tambahkan domain di dashboard Railway/Render, verifikasi DNS record yang di-setup dari Day 1 sudah propagate (cek pakai `dig` atau `nslookup`), pastikan SSL certificate aktif (biasanya otomatis via Railway/Render/Let's Encrypt, gak perlu setup manual).
**Definition of Done:** Domain company mengarah ke aplikasi production dengan HTTPS aktif, tidak ada warning certificate di browser.

### Card 2: Final smoke test end-to-end
**Deskripsi:** Jalankan checklist manual menyeluruh dari domain production: buka semua 5 halaman publik, cek semua fitur (map, filter kategori, link WA), login admin dan cek keempat modul CRUD (Promo, Branch, Product, News) berfungsi normal.
**Definition of Done:** Checklist lengkap dijalankan tanpa bug kritis; bug minor/kosmetik boleh dicatat untuk perbaikan pasca-launch.

### Card 3: Fix bug kritis — JANGAN nambah fitur baru
**Deskripsi:** Kalau smoke test menemukan bug yang menghalangi fungsi utama (bukan kosmetik), perbaiki sekarang. Tahan godaan untuk "sekalian nambahin" fitur baru di hari terakhir — risiko regresi terlalu tinggi menjelang deadline.
**Definition of Done:** Semua bug kritis (blocking) teratasi; tidak ada perubahan scope/fitur baru yang masuk hari ini.

### Card 4: Handoff notes buat atasan
**Deskripsi:** Tulis ringkasan singkat (bisa 1 halaman) untuk atasan: apa yang sudah live & berfungsi, apa yang sengaja ditunda ke fase berikutnya (lihat daftar Icebox), dan cara akses admin panel.
**Catatan:** Kirim URL + kredensial admin lewat kanal yang aman (bukan chat/grup biasa) — detail kecil ini juga bagian dari profesionalisme.
**Definition of Done:** Ringkasan terkirim ke atasan sebelum/tepat di deadline 20 September; kredensial admin disampaikan lewat kanal aman.
